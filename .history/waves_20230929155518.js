...
function createRandomStars(){
  ...
  for (let i = 0; i < numStars; i++) {
   ...
   container.classList.add("h-10","w-10");
   //container.classList.add("h-10","w-10",`animate-pulse`);
    container.style.position = "absolute";
    container.style.top = `${top}%`;
    container.style.left = `${left}%`;
    container.style.animation = `pulsenew 3s ease-in-out infinite`
   
    heroSection.appendChild(container);
}
}

function addOverLayToHero(){
 
  const container = document.createElement('div');
  container.classList.add("overlay","absolute", "top-0","h-full","w-full","transitionBg")
  heroSection.appendChild(container);
}
