...
async function settlePool(settlingTimePrice, endTimePrice) {
    ...
    const distributions = winners.map((user) => {
        totaDis += ((user.amount/winningPoolTotal) * pool.totalBets);
        return {
            ...user,
    ...
    console.log("distribyutions",totaDis, distributions)

     //POOL
    //update loosers database

    //update winners database 

    //USER PROFILE UPDATE WITH BALANCE
